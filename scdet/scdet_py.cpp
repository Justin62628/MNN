...
public:
    SceneDetector(const std::string &model_path)
        ...
        ipconfig.sourceFormat = RGB;
        ipconfig.destFormat   = BGR;
        ...
        py::array_t<uint8_t, py::array::c_style | py::array::forcecast> img1) 
    {
        ...
        int H = buf0.shape[0], W = buf0.shape[1], C = buf0.shape[2];
        uint8_t* ptr0 = static_cast<uint8_t*>(buf0.ptr);
        ...
        float *data = result.host<float>();
        int stride = result.stride(1);

        return { data[0], data[stride] };
    }
...
private:
    ...
    std::unique_ptr<Tensor> temp_host0;
    std::unique_ptr<Tensor> temp_host1;
